<?php
	...
	if( isset( $_POST['status'] ) ){
		...
		$status = $_POST['status'];
		$id = $_POST['id'];
		$status_update = "UPDATE customers SET status = '$status' WHERE id = '$id'";
...
<body>
	...
	<div class="container-fluid">
		...
		<h1>
			Customer Archives
			<a href="<?php echo $site_base; ?>admin/customers.php" class=" pull-right btn btn-default" ><< Go to Customers</a>
		</h1>
		...
		<?php 
			...
			// page check
			$pguery = "SELECT * FROM customers WHERE deleted = 1 ORDER BY id DESC";
			...
			{	
				if($_GET["page"] > $pcount)
				{
					$query = "SELECT * FROM customers WHERE deleted = 1 ORDER BY id DESC LIMIT 0 , 8";
				}else{
					$plim = (intval($_GET["page"])-1) * 8;
					$query = "SELECT * FROM customers WHERE deleted = 1 ORDER BY id DESC LIMIT ".$plim." , 8";
				}
			}else{
				$query = "SELECT * FROM customers WHERE deleted = 1 ORDER BY id DESC LIMIT 0 , 8";
			}
				...
				while($row2=mysqli_fetch_array($result2)){ $img_loc = $row2['location']; }
				    ...
				    <div class='col-lg-12 well'>
				        ...
				        -->
				        	...
				        	<a href="<?php echo $site_base . "admin/delete_any.php?type=customers&id=" . $row["id"]; ?>" class="btn btn-danger btn-block">Delete</a>
		...
		<div class="col-sm-12">
			<ul class="pagination">
				<?php 
					...
					$query = "SELECT * FROM customers WHERE deleted = 1 ORDER BY id DESC";
