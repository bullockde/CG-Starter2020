<?php
	...
	$action = $_GET["action"]; // Delete , Add, Update, Archive
	$id = $_GET["id"]; 
	$type = $_GET["type"]; 
	$title = "";
	...
	if( ($action == "Text") && isset($_POST['Confrm']) ) 
	{
		echo "HERE!";
		$query = "SELECT * FROM ".$type." WHERE id = '$id'";
		$result = mysqli_query($link,$query);
		...
		header("Location: ".$site_base."admin/send-reviews.php?type=".$text_type."&p=".$phoneNum."");
		
		//send-reviews.php?text_type=regular&p=267-819-5247
			...
			<div class="col-md-12">
				<form method="post" role="form" enctype="multipart/form-data">
					<div class="form-group">
						<h3><?php echo $action; ?> <?php echo $title; ?> from <?php echo $type; ?>?</h3>
					</div>

					<input type="hidden" name="text_type" value="<?php echo $_GET["text_type"]; ?>" ></input>
					<input type="hidden" name="id" value="<?php echo $_GET["id"]; ?>" ></input>

					<div class="form-group">
						<input type="submit" name="Confirm" value="Confirm" class="btn btn-success form-control" />
					</div>
				</form>
			</div>
		</div>
